...
$wt_home = dirname(__FILE__) . '/wt-key-format.test-db';
$wt_uri = 'table:key-format';
...
foreach ($formats as $fmt => $value) {
    ...
    $uri = $wt_uri . '-' . $fmt;
    $db->create($uri, 'key_format=' . $fmt . ',value_format=' . $fmt);
    ...
    usort($valid, 'WiredTiger\cmp');
    usort($ret, 'WiredTiger\cmp');
